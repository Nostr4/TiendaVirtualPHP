...
include ("bloques/doctype.php");
            ...
            <?php
                try {    
                    $stmt = $con->prepare("SELECT * FROM articulos where UnidadesV > 0 ORDER BY UnidadesV DESC");
                        if (isset($_GET['orden'])) {
                            $orden = $_GET['orden'];
                            if ($orden == "UnidadesV") {
                                $campo = "UnidadesV";
                            ...
                            $stmt = $con->prepare("SELECT * FROM articulos where UnidadesV > 0 ORDER BY $campo $dir");
                        } else {
                            $stmt = $con->prepare("SELECT * FROM articulos where UnidadesV > 0 ORDER BY UnidadesV DESC");
                        }
                    ...
                    if (count($resultados) > 0) {
                        ...
                        <th>Precio</th>
                        <th>Stock</th>
                        <th><a href='?orden=UnidadesV&dir=".($dir == 'ASC' ? 'DESC' : 'ASC')."'>Unidades Vendidas ↕</a>
                        <th>Beneficio</th>";
